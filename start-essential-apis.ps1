# Start only essential APIs for module functionality
Write-Host "Starting essential API services..." -ForegroundColor Cyan

# Kill any existing processes on these ports
$ports = @(5100, 5142, 5143)
...
# Start Module Registry (required for module discovery)
...
Write-Host "`n✓ Essential services starting..." -ForegroundColor Cyan
Write-Host "  - ModuleRegistry.API: http://localhost:5100" -ForegroundColor White
Write-Host "  - Inventory.API: http://localhost:5142" -ForegroundColor White
Write-Host "  - Sales.API: http://localhost:5143" -ForegroundColor White
Write-Host "`nServices will open in separate PowerShell windows." -ForegroundColor Cyan
Write-Host "Close those windows to stop services." -ForegroundColor Yellow
