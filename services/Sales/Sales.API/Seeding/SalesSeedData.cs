...
namespace Sales.API.Seeding;

// DTOs used only for deserializing cross-module demo data fetched from Inventory/CRM APIs during seeding.
...
/// <summary>
/// Dev/demo-only in-memory seed data for the Sales module. Deliberately kept out of
...
/// so every module's Program.cs wires up seeding the same way: `new {Module}Seeder(...).SeedAsync()`.
/// </summary>
public class SalesSeeder : IModuleSeeder<SalesDbContext>
{
    // Default demo tenant CompanyId, shared across all modules via BusinessAsUsual.Infrastructure.SeedData.DemoTenant
    // so cross-module demo data (Sales, Inventory, CRM, etc.) always lines up under the same "tenant".
    public static readonly Guid DemoCompanyId = DemoTenant.CompanyId;
    private const string DemoTenantDb = "TestTenant";

    private readonly SalesDbContext _context;
    private readonly IHttpClientFactory _httpClientFactory;

    public SalesSeeder(SalesDbContext context, IHttpClientFactory httpClientFactory)
    {
        _context = context;
        _httpClientFactory = httpClientFactory;
    }

    // Explicit IModuleSeeder<SalesDbContext> implementation: this seeder is constructed with
    // its DbContext (and IHttpClientFactory) up front, so the interface's context parameter
    // is only honored here for contract compliance/testability; the constructor-provided
    // context remains the source of truth. Callers should normally use the parameterless
    // SeedAsync() below.
    Task IModuleSeeder<SalesDbContext>.SeedAsync(SalesDbContext context) => SeedAsync();

    public async Task SeedAsync()
    {
        var context = _context;
        var httpClientFactory = _httpClientFactory;
        if (context.Quotes.Any()) return;
        ...
        try
        {
            using var request = new HttpRequestMessage(HttpMethod.Get, "api/inventory/products");
            request.Headers.Add("X-Company-Id", DemoCompanyId.ToString());
            request.Headers.Add("X-Tenant-Db", DemoTenantDb);
            var response = await inventoryClient.SendAsync(request);
        ...
        foreach (var q in new[] { quote1, quote2, quote3 })
        {
            q.CompanyId = DemoCompanyId;
        }
        foreach (var o in new[] { order1, order2, order3, order4 })
        {
            o.CompanyId = DemoCompanyId;
        }
